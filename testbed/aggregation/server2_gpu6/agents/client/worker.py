...
class Worker:
    def __init__(
        ...
        #exporting RESIST shim layer
        self.shim = shimLayer(self.id, self.veth, self.veth2)
    ...
    def run(self):
        ...
        print(self.shim.get_if())
        clock(self.id, 0, self.shim.get_if())
        print("Joined!")
...
#        ).to_csv(self.out_file)
    ...
    def get_new_model(self, step, num_splits, parameter_shapes, total_size):
        ...
        print("-------reading rows------")
        flat_grad = read_rows(num_splits, self.id, step, self.shim.get_if())[:total_size]
        print(flat_grad)
        waiting_end = time.time()
        ...
        print("I do have the current model")
    ...
    def send_gradients(self, step, current_model, num_splits, veth):
        with torch.no_grad():
            ...
            inc_rows(num_splits, new_grads, self.id, step, veth)

    def signal_clock(self, step):
        clock_wait_start = time.time()
        clock(self.id, step, self.veth)
...
def get_args():
    ...
    parser.add_argument("--veth", type=str)
